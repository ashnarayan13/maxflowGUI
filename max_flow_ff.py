...
class FlowNetwork:
    ...
    def addVertex(self, name, source=False, sink=False):
        '''Adds vertex to the graph. Checks if the vertext is source / sink or
        if it is normal node in the graph'''
    ...
    def addEdge(self, start, end, capacity):
        '''Adds an edge between 2 nodes with weight = capacity'''
        if start == end:
...
nodes = int(raw_input('Enter the number of nodes (excluding S and T)'))
print 'Enter the vertex names'
for i in range(0, nodes):
    temp = str(raw_input())
    fn.addVertex(temp)
edges = int(raw_input('Enter the number of edges (including edges from source and sink)'))
print 'Enter the edges, (source , destination, weight)'
for i in range(0, edges):
    temp = raw_input().split()
    fn.addEdge(temp[0], temp[1], int(temp[2]))
